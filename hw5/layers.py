...
def affine_forward(x, theta, theta0):
  ...
  #print x.shape
  #rX=x.reshape(x.shape[0], np.prod(x.shape[1:]))
  rX=x.reshape(x.shape[0], -1)
  #print reshapedX.shape
  out = rX.dot(theta) + theta0
...
def conv_forward_naive(x, theta, theta0, conv_param):
  ...
  out = np.zeros([m, F, H_, W_])
  X = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')

  for f in xrange(F):
    theta_ = theta[f].reshape(C*HH*WW)
    for i in xrange(m):
      for h in xrange(0, H, stride):
        for w in xrange(0, W, stride):
          x_ = X[i, :, h:h + HH, w:w + WW].reshape(C * HH * WW)
          out[i, f, h / stride, w / stride] = theta_.dot(x_.T) + theta0[f]
...
def conv_backward_naive(dout, cache):
  ...
  for f in xrange(F):
    ...
    for i in xrange(m):
      for h in xrange(0, H, stride):
        for w in xrange(0, W, stride):
          x_ = X[i, :, h:h + HH, w:w + WW]
          dout_=dout[i, f, h/stride, w/stride]
          dtheta0[f] += dout_
          for hh in xrange(HH):
            for ww in xrange(WW):
              dx[i, :, h+hh, w+ww] += dout_ * theta_[:, hh, ww]
              dtheta[f, :, hh, ww] +=dout_ * x_[:, hh, ww]
  dx = dx[:, :, 1:-1, 1:-1]
  ...
  for i in xrange(m):
    for h in xrange(0, H, S):
      for w in xrange(0, W, S):
        for c in xrange(C):
          out[i, c, h/S, w/S] = np.max(x[i, c, h:h + pH, w:w + pW])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in xrange(m):
    for h in xrange(0, H, S):
      for w in xrange(0, W, S):
        for c in xrange(C):
          maxh, maxw = np.unravel_index(np.argmax(x[i, c, h:h + pH, w:w + pW]), [pH, pW])
          dx[i, c, h + maxh, w + maxw] = dout[i, c, h / S, w / S]
