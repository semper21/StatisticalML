...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    l1out, cache1 = affine_relu_forward(X, self.params['theta1'], self.params['theta1_0']) #affine-relu
    scores, cache2 = affine_forward(l1out, self.params['theta2'], self.params['theta2_0']) #affine
    ...
    if y is None:
      return scores
    #print np.max(scores), np.max(self.params['theta1']), np.max(self.params['theta2']), np.max(X)
    loss, grads = 0, {}
    ############################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(self.num_layers):
        self.params['theta' + str(i + 1)] = np.random.randn(dims[i], dims[i + 1]) * weight_scale
