...
class one_vs_allLogisticRegressor:
    ...
    def train(self,X,y,reg):
       ...
       """
        ...
        ###########################################################################
        for l in self.labels: #from 1 to 10
            lr = linear_model.LogisticRegression(penalty = 'l2', C = 1e42/reg, fit_intercept = False, solver='lbfgs')
            lr.fit(X, [1 if j == l else 0 for j in y]) #1 for that class, 0 for all other classes
            theta_opt[:,l-1] = lr.coef_
