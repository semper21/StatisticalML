...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        '''
        r = np.dot(X, self.theta)-y
        J = (r.dot(r) + reg(self.theta.dot(selt.theta)))/(2*y.shape[0])
        '''
        J = 1. / (2 * num_examples) * sum((y - X.dot(theta)) ** 2)  # loss term
        J += reg / (2 * num_examples) * (sum([j ** 2 for j in theta[1:]]))  # penalty
    ...
    def grad_loss(self,*args):                                                                          
        ...
        '''
        r = X.dot(theta) - y
        grad = (r.transpose().dot(X) + reg.dot(self.theta))/y.shape[0]
        '''
        for j in xrange(dim):
            grad[j] = 1. / num_examples * sum((x.dot(theta) - y[i]) * x[j] for i, x in enumerate(X))
            grad[j] += 0 if j == 0 else reg / num_examples * theta[j]
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
